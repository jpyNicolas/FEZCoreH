...
class LOGADJUSTCalculator(Base):
    ...
    def calculate(self, extra_params):
        ...
        if extra_params is None and type(extra_params) is not dict:
            raise HTTPException(
                detail="you should send value of (gain and inv), these params are required",
                status_code=status.HTTP_412_PRECONDITION_FAILED,
            )
        if extra_params.get("gain") is None or extra_params.get("inv") is None:
            raise HTTPException(
        ...
        log_adjust = exposure.adjust_log(float_image, gain=gain, inv=inv)
    ...
    def export(self, file_path, title):
        ...
        ax[0].imshow(self.log_adjust, cmap="gray")
        ax[0].set_title(f"Log Adjust {title}")
        ax[0].axis("off")
        ax[1].hist(
            self.log_adjust.ravel(),
            bins=256,
            density=True,
            histtype="bar",
            color="black",
...
if __name__ == "__main__":
    ...
    imageenhancement = calculator.calculate({"inv": False, "gain": 1})
